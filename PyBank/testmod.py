def print_out(dividend, divisor, last_dividend_mult, last_divisor_mult,
              dividend_mult, divisor_mult, f_len=0, g_len=0):

        f_divs = "{0:{6},} {1:{6},}    {2:{6},}{3:{6},}  {4:{6},}{5:{6},}"
        arg_divs = [dividend, divisor, last_dividend_mult,
                    last_divisor_mult, dividend_mult,
                    divisor_mult, f_len]

        f_Euclid = "    {0:{4},} = {1:,}*({2:,}) + {3:,}"
        arg_Euclid = [dividend, dividend // divisor, divisor,
                      dividend % divisor, f_len]
...
def gcds(dividend,
        ...
        if print_it:
            if not dividend_mult:
                ...
                f_len += f_len // 3
                g_len = len(str("{0:{4},} = {1:,}*({2:,}) + {3:,}".format(
                    dividend,
                    dividend // divisor,
                    divisor,
                    dividend % divisor,
            ...
            print_out(
                dividend, divisor, last_dividend_mult, last_divisor_mult,
                dividend_mult, divisor_mult, f_len, g_len)
        ...
        if remainder:
            ...
            if print_it:
                ...
                arg_terminate = [divisor, remainder, ' ',
                                 "END OF EUCLIDEAN ALGORITHM",
                                 f_len, 4*f_len + 6, g_len + 5]
